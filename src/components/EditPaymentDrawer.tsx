...
import { type PaymentHistory, type PaymentMethodType } from '../types';
import { db } from '../db/database';
import { updatePaymentTransaction } from '../lib/businessLogic';
...
export function EditPaymentDrawer({ payment, isOpen, onClose }: EditPaymentDrawerProps) {
  ...
  useEffect(() => {
    if (isOpen && payment) {
      ...
      setAmount(payment.costIncurred);
      setMethod(payment.paymentMethodType);
      setDatePaid(payment.datePaid);
      setSelectedCardId(payment.chargedToItemId);
    }
  }, [isOpen, payment]);
      ...
      >
        <div className="glass-panel max-h-[90vh] md:h-full md:max-h-screen rounded-t-3xl md:rounded-t-none md:rounded-l-3xl p-6 flex flex-col relative border-b-0 md:border-r-0 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] md:shadow-[-10px_0_40px_rgba(0,0,0,0.5)]">
          ...
          <div className="flex-shrink-0">
            ...
            <h2 className="text-2xl font-bold mb-1 text-white pr-10">Edit Payment</h2>
